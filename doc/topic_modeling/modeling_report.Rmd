...
```{r topic_prevalence, fig.width=10, fig.height=4, fig.align="center"}
...
```{r econ_difference, fig.width=7, fig.height=4, fig.align="center"}
...
```{r}
stmprinter::plot_frex(stm_model)
```

```{r}
stmprinter::plot_thoughts(stm_model, docs$reconstructed_chars, n_sentences=25, split_width=62, cutoff = 190)
```
...
```{r}
# cleanup environment
rm(list=setdiff(ls(), c("out", "stm_model", "ee")))
# push down image
save.image("csr_stm_8.RData")
```
